...
function AssignmentDeleteModal({ show, assignment, onClose, onDelete, onExport }) {
  ...
  const handleDelete = async () => {
    const confirmed = window.confirm(
      `确定要删除作业"${assignment.name}"及其所有 ${assignment.submission_count || 0} 个作品吗？\n\n此操作不可恢复！`
    );
    ...
    setIsDeleting(true);
    try {
      await axios.delete(`/api/assignments/${assignment.id}/cascade`);
      onDelete();
      onClose();
    } catch (err) {
          ...
          <div className="modal-body">
            <div className="alert alert-warning" role="alert">
              <div className="d-flex">
                ...
                <div>
                  <h4 className="alert-title">该作业有 {assignment.submission_count || 0} 个相关作品</h4>
