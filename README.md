...
## ⚠️ 重要：服务器启动说明（AI助手请先阅读）

当需要启动服务器时，如果遇到端口占用或连接问题，请务必先阅读并遵循以下步骤：
...
此脚本会：
1. 自动终止可能存在的前端（端口3000）、后端（端口5000）服务器进程
2. 然后统一启动整个系统
...
```bash
# 终止占用端口的进程
lsof -ti:5000 | xargs kill -9  # 终止后端进程
lsof -ti:3000 | xargs kill -9  # 终止前端进程
